...
# Mock the exit function to prevent the script from exiting during tests
exit() {
  _last_exit_code=$1
}
...
printf() {
  ...
  # Add a temporary marker to prevent trimming of trailing newlines
  formatted_output="$(command printf -- "$@"; printf '%s' "_SHUNIT2_NL_PRESERVER_")"
  # Remove the marker
  formatted_output="${formatted_output%_SHUNIT2_NL_PRESERVER_}"
...
mock_read() {
  if [[ -n "${_mock_input_array[*]}" ]]; then
    ...
    # Fallback to the original read if no mock input is set or the array is empty
...
# Helper function to reset captured output and mocks
reset_mocks() {
  _last_exit_code=0
...
test_main_rebase_specific_commit_nonexistent() {
  reset_mocks
  # Mock git rev-parse to simulate a non-existent commit
  # shellcheck disable=SC2317
  git() {
...
test_main_rebase_specific_commit_cat_file_fail() {
  reset_mocks
  # Mock git cat-file to simulate a non-existent commit
  # shellcheck disable=SC2317
  git() {
    if [[ "$1" == "cat-file" ]]; then
      return 1
...
test_handle_paused_rebase_continue_success() {
  reset_mocks
  # Simulate that the rebase-merge directory exists
  mkdir "$_test_git_root/.git/rebase-merge"
  
  set_mock_input "c"
  handle_paused_rebase "$_test_git_root"
  assertEquals "0" "$_last_exit_code"
  assertFalse "[[ -d \"$_test_git_root/.git/rebase-merge\" ]]" # Should be removed after successful continue
  assertTrue "[[ \"$_captured_printf_output\" =~ \"Git Rebase operation completed.\" ]]"
}

test_handle_paused_rebase_continue_fail() {
  reset_mocks
  # Simulate that the rebase-merge directory exists and git rebase --continue fails
  mkdir "$_test_git_root/.git/rebase-merge"
...
test_handle_paused_rebase_abort() {
  ...
  handle_paused_rebase "$_test_git_root"
  assertEquals "0" "$_last_exit_code"
  assertFalse "[[ -d \"$_test_git_root/.git/rebase-merge\" ]]" # Should be removed after abort
...
test_main_post_rebase_stash_reminder() {
  reset_mocks
  # Mock git diff to simulate uncommitted changes
  # Mock git stash to simulate successful stash
  # shellcheck disable=SC2317
  git() {
    case "$1" in
      "rev-parse")
      ...
      "status")
        ...
        ;;
      "cat-file")
        return 0 # Simulate commit existence
        ;;
      *)
        printf '%s' "mock git command: $*" >&2
        return 1
  ...
  set_mock_input "s" "1" "n" # Stash, then rebase from root, then no (for `Press Enter to continue`)
  main "--default"
  assertEquals "0" "$_last_exit_code"
  assertTrue "[[ \"$_captured_printf_output\" =~ \"Remember: You stashed changes before starting the script.\" ]]"
  assertTrue "[[ \"$_captured_printf_output\" =~ \"Please restore them using 'git stash pop' after you are done.\" ]]"
}

test_main_post_rebase_force_push_reminder() {
  reset_mocks
  # Mock git diff to simulate no uncommitted changes
  # Mock git rebase to simulate successful rebase
  # shellcheck disable=SC2317
  git() {
    case "$1" in
      "rev-parse")
        ...
        ;;
      "diff")
        return 0 # Simulate no changes
        ;;
      "stash")
        return 0 # Should not be called if no changes
        ;;
      "rebase")
        return 0 # Simulate successful rebase
        ;;
  ...
  set_mock_input "1" "n" # Rebase from root, then no (for `Press Enter to continue`)
  main "--default"
  assertEquals "0" "$_last_exit_code"
  assertTrue "[[ \"$_captured_printf_output\" =~ \"If you have successfully rewritten the commits, you may need to\" ]]"
  assertTrue "[[ \"$_captured_printf_output\" =~ \"force push changes to the remote repository.\" ]]"
  assertTrue "[[ \"$_captured_printf_output\" =~ \"Please check your Git\" ]]"
  assertTrue "[[ \"$_captured_printf_output\" =~ \"history with 'git log --oneline' and, if necessary, execute\" ]]"
  assertTrue "[[ \"$_captured_printf_output\" =~ \"'git push --force-with-lease'. Remember, this is only needed if you\" ]]"
  assertTrue "[[ \"$_captured_printf_output\" =~ \"have already pushed these commits to a remote branch.\" ]]"
}

test_main_not_in_git_repo() {
  reset_mocks
  # Temporarily override git rev-parse to simulate not being in a repository
  # shellcheck disable=SC2317
  git() {
    if [[ "$1" == "rev-parse" ]]; then
      echo ""
